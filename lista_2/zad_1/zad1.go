...
func nodeWork(id int, channels []chan *packet, myqueue []int, msgChan chan<- string, doneChan <-chan bool, trashChan chan<- *packet) {
...
func main() {
	...
	for i := 0; i < n; i++ {
		go nodeWork(i, channels, Nqueues[i], msgChan, doneChan, trashChan)
	}
	...
	sender := func() {
		for i := 0; i < k; i++ {
			time.Sleep(time.Second * randDuration())
			newPacket := packet{id: i, visitedNodes: make([]int, 0), life: h}
